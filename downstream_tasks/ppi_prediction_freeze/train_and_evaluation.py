"""
Script to train the classification head on pre-generated embeddings for PPI prediction.
This script loads embeddings and labels, then trains a classification head for sequence-level PPI prediction.
"""
import torch
...
class PPIDataset(Dataset):
    """
    Dataset class for PPI embeddings and labels (sequence-level classification).
    """
    ...
    def __getitem__(self, idx):
        ...
        @returns: Tuple of (embeddings, label) where embeddings is a tensor and label is a scalar
        """
...
def collate_fn(batch):
    ...
    @param batch: List of (embeddings, label) tuples
    @returns: Tuple of (padded_embeddings, labels, attention_mask)
    """
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for embeddings, labels, attention_mask in tqdm(dataloader, desc="Training"):
        ...
        # Forward pass
        logits = model(embeddings, attention_mask=attention_mask)  # Shape: [batch_size, num_labels]
...
def main():
    parser = argparse.ArgumentParser(description="Train classification head for PPI prediction")
    parser.add_argument(
        "--embeddings_dir",
        type=str,
    ...
    train_dataset = PPIDataset(train_embeddings, train_labels)
    ...
    if valid_embeddings is not None and valid_labels is not None:
        valid_dataset = PPIDataset(valid_embeddings, valid_labels)
    ...
    if test_embeddings is not None and test_labels is not None:
        test_dataset = PPIDataset(test_embeddings, test_labels)
    ...
    # Initialize model
    print("🚀 Initializing PPI classification head...")
    model = PPIClassificationHead(
