#!/bin/bash
# Script to generate embeddings for PPI prediction
...
if [ $# -eq 0 ]; then
    ...
    echo "  $0 facebook/esm2_t30_150M_UR50D 12 5     # Use layer 12, GPU 5"
    exit 1
fi

PRETRAINED_MODEL_NAME=$1
LAYER_INDEX_ARG=""
GPU_ID="5"  # Default GPU
...
echo "🚀 Generating embeddings for PPI prediction..."
...
# Run PPI prediction embedding generation
python utils/embeddings_generator/ppi_generator.py --pretrained_model_name "$PRETRAINED_MODEL_NAME" $LAYER_INDEX_ARG

echo "✅ PPI prediction embeddings generation completed!"