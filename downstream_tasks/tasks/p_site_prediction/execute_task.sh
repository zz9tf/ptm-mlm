...
if [ $# -ne 4 ]; then
    ...
    echo "Example: $0 LoRA_combine_ptm EvolutionaryScale_esmc-600m-2024-12 30 0"
    echo "Example (no checkpoint): $0 None EvolutionaryScale_esmc-600m-2024-12 30 0"
    exit 1
fi
...
# 配置参数 (Configuration)
# ============================================
WORK_DIR="/home/zz/zheng/ptm-mlm/downstream_tasks/tasks/p_site_prediction"
BASE_OUTPUT_DIR="/home/zz/zheng/ptm-mlm/downstream_tasks/outputs"

# 创建带日期的输出目录
DATE_STR=$(date +"%Y-%m-%d")
OUTPUT_DIR="${BASE_OUTPUT_DIR}/p_site_prediction_${ADAPTOR_CHECKPOINT}_${DATE_STR}"
...
echo "🔄 Starting p_site prediction pipeline..."
if [ -z "$ADAPTOR_CHECKPOINT" ] || [ "$ADAPTOR_CHECKPOINT" = "None" ] || [ "$ADAPTOR_CHECKPOINT" = "none" ]; then
...
python train_and_evaluation.py "${PYTHON_ARGS[@]}"
