...
# Import from local source to get Block class
import sys
from pathlib import Path
_local_conv1d_path = Path(__file__).parent.parent / "libs" / "causal-conv1d"
_local_mamba_path = Path(__file__).parent.parent / "libs" / "mamba"
if str(_local_conv1d_path) not in sys.path:
    # Prefer the installed causal_conv1d (newer API) before falling back to the vendored copy.
    sys.path.append(str(_local_conv1d_path))
if str(_local_mamba_path) not in sys.path:
    sys.path.insert(0, str(_local_mamba_path))
from mamba_ssm.modules.mamba_simple import Mamba, Block
# Import from installed package for other utilities
